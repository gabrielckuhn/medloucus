...
def get_image_as_base64(path):
    """Retorna a string base64 da imagem ou None se der erro/não existir"""
    try:
...
st.markdown("""
    ...
    /* --- ESTILO ESPECÍFICO ÁREA NETFLIX (CORRIGIDO) --- */
    .netflix-container {
        display: flex;
    ...
    .netflix-profile-link {
        ...
        align-items: center;
        width: 110px; /* Largura fixa para evitar colapso */
        cursor: pointer;
    ...
    /* Classe unificada para Imagem ou Div Colorida */
    .netflix-avatar-box {
        width: 100px;
        height: 100px;
        border-radius: 4px;
        ...
        /* Flex para centralizar letra se for div */
        display: flex; 
        align-items: center; 
        justify-content: center;
        font-size: 40px;
    ...
    /* Hover: Borda branca */
    .netflix-profile-link:hover .netflix-avatar-box {
...
if st.session_state['pagina_atual'] == 'dashboard':
    ...
    # ---------------------------------------------------------------------
    # SELEÇÃO DE PERFIL ESTILO NETFLIX (CORRIGIDO)
    # ---------------------------------------------------------------------
    ...
    for user in LISTA_USUARIOS:
        ...
        cor_usuario = USUARIOS_CONFIG[user]['color']
        
        # LÓGICA DE FALLBACK:
        # Se img_b64 for válido, cria uma tag <img>
        # Se for None, cria uma <div> colorida com a inicial do nome
        
        if img_b64:
            avatar_html = f'<img src="{img_b64}" class="netflix-avatar-box">'
        else:
            inicial = user[0]
            avatar_html = f'<div class="netflix-avatar-box" style="background-color: {cor_usuario};">{inicial}</div>'
...
# =========================================================
    ...
    with c_back:
        if st.button("⬅", help="Voltar"):
            ir_para_dashboard()
    with c_head:
        # Header Perfil (se não tiver imagem, usa uma bolinha colorida)
        if img:
            img_html = f'<img src="{img}" class="profile-header-img" style="border-color:{cor}">'
        else:
            img_html = f'<div class="profile-header-img" style="border-color:{cor}; background:{cor}; display:flex; align-items:center; justify-content:center; color:white; font-size:30px; font-weight:bold;">{user[0]}</div>'
